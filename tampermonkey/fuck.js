...
(function() {
  ...
  data('显示').watch(flag => {
    if (flag) {
      style.sheet.deleteRule([...style.sheet.rules].findIndex(rule => {
  ...
  const observer = {
    ...
    watch(ancestor, type, selector, handler, config = {}) {
      ...
      observe(ancestor).addChild(node => document.contains(node) && handler[0] && handler[0](ancestor, selector, node, opts), config.subtree)
      this.run()
  ...
  async function fuck() {
    if (checkHref('.zhihu.com')) {
      ...
      observer.in('#juejin', '.sidebar-bd-entry', { remove: true })
      observer.in('#juejin', '.guide-collect-popover', { remove: true })
      observer.in('#juejin', '.main-header-box', { remove: true })
    } else if (checkHref('leetcode.cn')) {
    ...
    return {
      ...
      async fillInfo() {
        ...
        const dates = Array.from(document.querySelectorAll('[data-year-month]')).map(node => {
          ...
          return {
            node,
            text: node.getAttribute('data-year-month').trim(),
