  ...
  while (true) {
    for (const session of sessions) {
      if (session.remaining === -1) {
        console.log('\033[2m', session.summary, '售罄', '\033[0m')
        // await sleep(1)
        // continue
      } else {
        console.log('\033[33m', session.summary, '可售', '\033[0m')
      }
      console.log('\033[2m', '抢票', new Date().toLocaleTimeString(), '\033[0m', session.summary)
      const res = await submit(env.MODULEID, session)
      if (res.code === 'A00005') {
        console.log('\033[31m', res.code, res.errmsg, '\033[0m')
        ...
        console.log('\033[32m', res.code, res.errmsg, '\033[0m')
      }
      await sleep(gap)
      console.log(gap)
    }
