...
TOUTIAO_HOT_URL='https://tsearch.snssdk.com/search/suggest/hot_words/'
...
function weibo() {
  ...
  local i=0
  case ${1:-hot} in
    resou|hot) url="$WEIBO_HOT_URL";;
    yaowen|vip) url="$WEIBO_YAOWEN_URL";;
    wenyu) url="$WEIBO_WENYU_URL";;
  esac
...
function json() {
  ...
  echo "${fieldPatterns[@]} ${fieldIndexes[@]}"
  local titles=()
  local links=()
...
function zhihu() {
  ...
  local domain="$2"
  case ${type:=hot} in
    resou|hot) url="$ZHIHU_HOT_URL"; fieldPatterns=('"target":{.*?}' '"link":{.*?}'); fieldIndexes=(8 6);;
...
# https://www.toutiao.comhttps://tsearch.snssdk.com/search/suggest/hot_words/
  local url="$TOUTIAO_HOT_URL"
  local text=`curl -s "$url"`
  local fieldPatterns=('"query":".*?"' '"query":".*?"')
  local fieldIndexes=(4 4)
  json -s "$text" -p "${fieldPatterns[@]}" -i "${fieldIndexes[@]}"
}
